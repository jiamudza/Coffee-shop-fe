...
const Header = () => {
  ...
  useEffect(() => {
    if (localStorage.getItem("@isLogin")) {
      ...
      setIsLogin(true);
      axios.get(`https://coffeeshop-be.adaptable.app/api/v1/users/${userId}`)
      .then(res => {
        setUserData({
            ...userData,
  ...
  return (
    <div className="font-rubik bg-white pb-5 bg-opacity-90">
      <div
        className={
          drawerOpen
            ? "flex blur-sm md:blur-none pt-5 px-5 md:px-20 items-center justify-between"
            : "flex  pt-5 px-5 md:px-20 items-center justify-between"
        }
      >
        <button
          id={hamburgerMenu}
        ...
        {isLogin ? (
          <div className="flex items-center">
            ...
            <div className="flex cursor-pointer">
              {notif ? (
                <div className="w-2 h-2 bg-red-600 rounded-full absolute mx-3"></div>
              ) : (
                <div></div>
              )}
              <BsChatLeftText
                size={25}
                className="text-text mx-4 bg-transparent"
              />
            ...
            <div>
              <img
                ...
                src={userData.data.image !== null ? userData.data.image: userPlaceholder}
                className="w-10 rounded-full mx-5 cursor-pointer"
                alt="user-profile"
              />
          ...
          <div className="block">
            <Link
              to="/login"
              className="px-5 py-2 bg-Transparent mx-2 text-title font-bold rounded-3xl hover:bg-secondary hover:text-white ease-in duration-200"
            >
              Login
            </Link>
            <Link
              to="/register"
              className="px-5 py-2 bg-primary text-secondary font-bold rounded-3xl hover:bg-secondary hover:text-white ease-in duration-200"
            >
              Sign Up
            </Link>
          </div>
        ...
        {isLogin ? (
          <div className="flex flex-col items-start right-20 top-0 bg-transparent py-0 w-full text-center text-text">
            <div className="bg-secondary w-full rounded-r-xl">
              ...
              >
                <img
                  ...
                  alt="user-profile"
                />
                <p className="font-bold py-3">{isLogin ? userData.data.display_name : 'Anonimous'}</p>
                <p>{isLogin? userData.data.email : 'Anonimous'}</p>
              </div>
            </div>
            ...
            <div
              ...
              className="py-5 px-20 flex items-center justify-around cursor-pointer"
            >
            ...
            <div
              onClick={handleLogout}
              className="py-3 absolute right-2 bottom-10  flex items-center justify-around cursor-pointer"
            >
              <RiLogoutCircleLine size={25} className="text-text mx-3" />
              <p>Logout</p>
            </div>
          ...
          <div>
            ...
            >
              <AiFillHome size={25} className="text-text mx-3 " />
              <p>
                Home<span className="text-transparent">....</span>
              </p>
            </div>
            <div
              onClick={() => navigate("/product")}
            ...
            <div className="absolute bottom-20 w-full cursor-pointer flex justify-around px-10">
              <Link
                to="/login"
                className="px-5 py-2 bg-Transparent mx-2 text-title font-bold rounded-3xl hover:bg-secondary hover:text-white ease-in duration-200 focus:bg-none"
              >
                Login
              </Link>
              <Link
